...
def get_dg_data_loaders(batch_size, test_vendor='D', image_size=224):
    ...
    if test_vendor == 'A' or test_vendor == 'B':
        resampling_rate = 1.2
    else:
        resampling_rate = 1.1

    train_labeled_loader, train_labeled_dataset, train_unlabeled_loader, train_unlabeled_dataset, test_loader, test_dataset = get_data_loader_folder(Labeled_data_dir, Labeled_mask_dir, batch_size, image_size, test_num=test_vendor, resampling_rate=resampling_rate)
...
def get_data_loader_folder(data_folders, mask_folders, batch_size, new_size=224, test_num='D', num_workers=4, resampling_rate=1.2):
    if test_num=='A':
    ...
    train_labeled_dataset = ImageFolder(train_data_dirs, train_mask_dirs, ref_data_dirs, train_re, num_label=train_num, train=True, labeled=True, resampling_rate=resampling_rate)
    train_unlabeled_dataset = ImageFolder(train_data_dirs, train_mask_dirs, ref_data_dirs, train_re, num_label=train_num, train=True, labeled=False,resampling_rate=resampling_rate)
    test_dataset = ImageFolder(train_data_dirs, train_mask_dirs, ref_data_dirs, train_re, num_label=train_num, train=False, labeled=True, resampling_rate=resampling_rate)
...
class ImageFolder(data.Dataset):
    def __init__(self, data_dirs, mask_dirs, ref_dir,  re, train=True, num_label=None, labeled=True, loader=default_loader, resampling_rate=1.2):
    ...
    def __getitem__(self, index):
        ...
        if self.labeled:
            if self.train:
                ...
                resize_order = re / self.resampling_rate
                ...
                # if mask.sum() == 0:
                #     heart_label = torch.tensor([[0], [0], [0]])
                # elif mask[1].sum() == 0 and mask[2].sum() == 0:
                #     heart_label = torch.tensor([[1], [0], [0]])
                # elif mask[0].sum() == 0 and mask[2].sum() == 0:
                #     heart_label = torch.tensor([[0], [1], [0]])
                # elif mask[0].sum() == 0 and mask[1].sum() == 0:
                #     heart_label = torch.tensor([[0], [0], [1]])
                # elif mask[0].sum() == 0:
                #     heart_label = torch.tensor([[0], [1], [1]])
                # elif mask[1].sum() == 0:
                #     heart_label = torch.tensor([[1], [0], [1]])
                # elif mask[2].sum() == 0:
                #     heart_label = torch.tensor([[1], [1], [0]])
                # else:
                #     heart_label = torch.tensor([[1], [1], [1]])
                if mask.sum() == 0:
                    heart_label = torch.tensor([0])
                else:
                    heart_label = torch.tensor([1])


                heart_label = heart_label.type_as(mask)
