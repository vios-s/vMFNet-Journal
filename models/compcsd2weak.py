...
class CompCSD2weak(nn.Module):
    ...
    def forward(self, x, layer=7):
        ...
        # pre_seg = TVF.resize(norm_vmf_activations, ((2**(9-layer))*norm_vmf_activations.size(2), (2**(9-layer))*norm_vmf_activations.size(3)), interpolation=InterpolationMode.NEAREST)
        pre_seg = self.segmentor(norm_vmf_activations, features)
        # pred_heart = self.heartor(norm_vmf_activations[:, 0:3, :, :])
        pred_heart = self.heartor(pre_seg[:, 0:3, :, :])
        ...
        return rec, pre_seg, content, features[layer], kernels, norm_vmf_activations, pred_heart
