...
import utils
from eval_weak import eval_compcsd
from loaders.mms_dataloader_dg_aug_semiweaksep import get_dg_data_loaders
import models
...
def get_args():
    ...
    parser.add_argument('-mn','--model_name', type=str, default='compcsd2weak', help='Name of the model architecture to be used for training/testing.')
...
# python train_weakvmf.py -e 200 -bs 4 -c cp_compcsd2_100_tvA_weakvmf/ -enc cp_unet_100_tvA/UNet.pth -t A -w CompCSD_12_weakvmf_tvA2 -g 0
# k_un = 1
# k1 = 40
# k2 = 4

# python train_weakvmf.py -e 1200 -bs 4 -c cp_compcsd2_5_tvA_weakvmf/ -enc cp_unet_100_tvA/UNet.pth -t A -w CompCSD_12_weakvmf_tvA2 -g 0
# k_un = 1
# k1 = 400
# k2 = 40

# python train_weakvmf.py -e 2000 -bs 4 -c cp_compcsd2_2_tvA_weakvmf/ -enc cp_unet_100_tvA/UNet.pth -t A -w CompCSD_12_weakvmf_tvA2 -g 0
# python train_weakvmf.py -e 2000 -bs 4 -c cp_compcsd2_2_tvB_weakvmf/ -enc cp_unet_100_tvB/UNet.pth -t B -w CompCSD_12_weakvmf_tvB2 -g 2
# python train_weakvmf.py -e 2000 -bs 4 -c cp_compcsd2_2_tvC_weakvmf/ -enc cp_unet_100_tvC/UNet.pth -t C -w CompCSD_12_weakvmf_tvC2 -g 3
# python train_weakvmf.py -e 2000 -bs 4 -c cp_compcsd2_2_tvD_weakvmf/ -enc cp_unet_100_tvD/UNet.pth -t D -w CompCSD_12_weakvmf_tvD2 -g 0
k_un = 1
k1 = 600
k2 = 80
...
def train_net(args):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            ...
            for imgs, true_masks, heart_label in train_loader:
                # print(imgs.size())
                imgs = imgs.to(device=device, dtype=torch.float32)
                mask_type = torch.float32
                ce_mask = true_masks.clone().to(device=device, dtype=torch.long)
                true_masks = true_masks.to(device=device, dtype=mask_type)
                heart_label = heart_label.to(device=device, dtype=torch.float32)
                ...
                # heart_weak_loss = l1_distance(pred_heart, heart_label.squeeze(2))
                heart_weak_loss = l1_distance(pred_heart, heart_label)


                batch_loss = 0*reco_loss + 0*clu_loss  + loss_dice + loss_focal + 1*heart_weak_loss
                ...
                writer.add_scalar('loss/heart_weak_loss', heart_weak_loss.item(), global_step)
                ...
                for i in range(k_un):
                    # print('Un* start'*5)
                    un_imgs, _, un_heart_label= next(un_itr)
                    un_imgs = un_imgs.to(device=device, dtype=torch.float32)
                    un_heart_label = un_heart_label.to(device=device, dtype=torch.float32)

                    rec, pre_seg, content, features, kernels, L_visuals, pred_heart = model(un_imgs, layer=layer)

                    # un_heart_weak_loss = l1_distance(pred_heart, un_heart_label.squeeze(2))
                    un_heart_weak_loss = l1_distance(pred_heart, un_heart_label)

                    un_reco_loss = l1_distance(rec, un_imgs)
                    un_clu_loss = cluster_loss(features.detach(), kernels)

                    un_batch_loss = 0*un_reco_loss + 0*un_clu_loss + 1*un_heart_weak_loss
                    ...
                    writer.add_scalar('Loss_un/un_heart_weak_loss', un_heart_weak_loss.item(), un_step)
